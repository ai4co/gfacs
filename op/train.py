...
def train_instance(
    model, optimizer, batch_size, n_nodes, k_sparse, n_ants, invtemp=1.0, guided_exploration=False, beta=10.0, topk=5, it=0
):
    model.train()
    ##################################################
    # wandb
    ...
    for _ in range(batch_size):
        coor = torch.rand(size=(n_nodes, 2), device=DEVICE)
        ...
        aco = ACO(distances, prizes, MAX_LEN[n_nodes], n_ants, heuristic=heu_mat, device=DEVICE)
...
def train_epoch(
    ...
    for i in tqdm(range(steps_per_epoch), desc="Train"):
        ...
        train_instance(net, optimizer, batch_size, n_nodes, k_sparse, n_ants, invtemp, guided_exploration, beta, topk, it)
...
def train(
    ...
    val_list = load_val_dataset(n_nodes, k_sparse, DEVICE)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-o", "--output", type=str, default="../pretrained/op",
    ...
    if args.beta_min is None:
        beta_min_map = {100: 5, 200: 5, 500: 5}
        args.beta_min = beta_min_map[args.nodes]
    if args.beta_max is None:
        beta_max_map = {100: 10, 200: 10, 500: 10}
    ...
    run_name += f"op{args.nodes}_sd{args.seed}"
    pretrained_name = (
        args.pretrained.replace("../pretrained/op/", "").replace("/", "_").replace(".pt", "")
        if args.pretrained is not None else None
    )
    run_name += f"{'' if pretrained_name is None else '_fromckpt-'+pretrained_name}"
    if USE_WANDB:
        wandb.init(project="gfacs-op", name=run_name)
