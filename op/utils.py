...
def load_val_dataset(n_nodes, k_sparse, device):
    '''Return [(pyg_data, distance matrix, prizes)]
    '''
    val_list = []
    val_tensor = torch.load(f'../data/op/valDataset-{n_nodes}.pt')
...
def load_test_dataset(n_nodes, k_sparse, device):
    '''Return [(pyg_data, distance matrix, prizes)]
    '''
    val_list = []
    val_tensor = torch.load(f'../data/op/testDataset-{n_nodes}.pt')
    for coor in val_tensor:
        coor = coor.to(device)
        data, distances, prizes = gen_pyg_data(coor, k_sparse=k_sparse)
        val_list.append((data, distances, prizes))
