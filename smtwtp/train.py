...
from net import Net
from aco import ACO
from utils import gen_instance, load_val_dataset
...
def train_instance(
    ...
    for _ in range(batch_size):
        pyg_data, due_time, weights, processing_time = gen_instance(n_nodes, DEVICE)
        ...
        if guided_exploration:
            ...
            n = len(due_time)
            aco.pheromone = torch.ones(n + 1, n + 1, device=DEVICE)  # reset pheromone before calculating log_probs
...
if __name__ == "__main__":
    ...
    parser.add_argument("-o", "--output", type=str, default="../pretrained/smtwtp",
    ...
    run_name += f"smtwtp{args.nodes}_sd{args.seed}"
    pretrained_name = (
        args.pretrained.replace("../pretrained/smtwtp/", "").replace("/", "_").replace(".pt", "")
        if args.pretrained is not None else None
    )
    run_name += f"{'' if pretrained_name is None else '_fromckpt-'+pretrained_name}"
    if USE_WANDB:
        wandb.init(project="gfacs-smtwtp", name=run_name)
